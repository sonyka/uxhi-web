...
//
// Kept out of AgendaSection on purpose: the agenda is a sequence of time slots,
// and an item spanning five hours in no room would either break the time gutter
// or misrepresent itself as a session you pick instead of a talk. These are
// things you step out to do, so they read better as a pair of cards.
//
...
const SIDE_PROGRAMMING = [
  {
    ...
    body:
      "Thirty minutes of one-on-one time with an experienced designer, and honest feedback on the work you have been meaning to get fresh eyes on. Sign-ups open by email after you register, first come first served, and we fill any open slots with walk-ins. Bring your portfolio with you on the day.",
  },
  {
    time: "All day",
  ...
  return (
    <div className="flex flex-col gap-6 md:gap-8">
      ...
      */}
      ...
      {/* Side programming — runs concurrently with the talks. */}
      <div className="flex flex-col gap-3 md:gap-4">
        ...
        <p className={`${TYPE.body} max-w-[62ch]`} style={{ color: GRAY }}>
          Plenty going on beyond the sessions. Both of these ran at past
          UXHICons, you asked for them back, and they return this year. Step out
          of a talk and take your turn.
        </p>
        ...
        <ul className="grid gap-3 md:gap-4 grid-cols-[repeat(auto-fit,minmax(240px,1fr))] mt-1">
          {SIDE_PROGRAMMING.map((item) => (
            ...
            >
              {/* The agenda puts time in a left gutter. These span hours rather
                  than sitting at one, so the time moves into the card, keeping
                  the same weight and colour it has in the schedule. */}
              <div
              ...
              {/* Matches the agenda's session-title ramp exactly: these cards
                  sit a screen apart from those and read as the same kind of
                  thing. */}
